...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| **✅ Search Intelligence (Sprint 1+2)** — shipped 2026-05-04 (PR #186 + #187), capture + dashboard gaps + prompt L99                                                   | ~5j    | ROI direct : -50% no-result en 60j cible · base d'un module SaaS revendable (49-99 €/mois)                                    | `docs/SEARCH-INTELLIGENCE-ARCHITECTURE.md` |
| **✅ Rate limiting** sur POST publics — shipped 2026-05-04 (PR #190, in-memory per-IP, swap Upstash V2.2)                                                              | 1j     | Anti-DoS sur `/api/liste-scolaire/match` notamment (jusqu'à 80 FTS séquentielles par appel)                                   | audit security 2026-04-28 §3               |
| **✅ Sentry / observability** — shipped 2026-05-04 (PR #192, logger.ts forward `logError` vers Sentry quand `PUBLIC_SENTRY_DSN` set, fallback stderr toujours actif)   | 0.5j   | Aujourd'hui les erreurs vont à `process.stderr` Netlify Functions = visible mais pas alertable                                | `docs/SENTRY-SETUP.md`                     |
| **✅ Backup Supabase nightly** — shipped 2026-05-04 (PR #191, cron 03:00 UTC + trigger manuel, retention 14j) ; uptime monitoring reste à wirer côté Élie              | 0.5j   | Aujourd'hui aucun backup → 0 résilience si DB perdue                                                                          | `docs/SUPABASE-BACKUP.md`                  |
| **✅ Stock dual + sales_channel** — shipped 2026-05-04 (PR #196 + #197 + #198), approche SF1 cron nightly. Webhook quasi temps-réel SF2 reste backloggé V2.2 si besoin | ~4j    | Évite l'oversell e-commerce sur articles boutique-only ; débloque produits POS-only (services, vrac) sans pollution catalogue | `docs/STOCK-DUAL-CHECKLIST.md`             |
| **✅ CSP tightening** — shipped pre-cutover (PR #46) dans `netlify.toml`                                                                                               | 0.5j   | Audit "tighten in Phase 2" déjà tagué                                                                                         | `netlify.toml`                             |
| **✅ Lighthouse CI** — shipped pre-cutover (PR #47) dans `.github/workflows/lighthouse.yml`                                                                            | 1j     | Empêche les régressions perf invisibles. Cap ≥ 90 mobile sur PR                                                               | `.github/workflows/lighthouse.yml`         |
| **✅ Vrai OG image 1200×630 PNG** — shipped pre-cutover (PR #48 + #82)                                                                                                 | 0.5j   | Fallback SVG actuel passe Lighthouse mais Slack/FB rendent mal                                                                | `public/og-default.png`                    |

**V2.1 = COMPLET au 2026-05-04** ✅ (modulo setup secrets Élie : `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD`, `PUBLIC_SENTRY_DSN`, `SHOPIFY_LOCATION_ONLINE_ID`, `SHOPIFY_LOCATION_POS_ID` + curation produits `sales_channel='pos'` en Studio).
